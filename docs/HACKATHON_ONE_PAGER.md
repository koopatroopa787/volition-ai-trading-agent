...
An accepted order is labelled **submitted**, never filled. Volition polls broker order state and appends changes to a separate immutable lifecycle ledger. Filled positions are synced from Alpaca and checked for three bounded exit conditions: +25% of defined risk, −20% of defined risk, or five DTE. A qualifying exit reverses every leg with close intent in one atomic paper order. Active-order, existing-position, structure-count, symbol-risk and directional-risk checks prevent duplicate or concentrated entries. A completed exit does not suppress management of a later position in the same symbol.
