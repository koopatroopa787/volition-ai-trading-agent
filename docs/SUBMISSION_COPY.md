...
The deployed committee runs privately on Qwen 2.5 3B through Ollama. The model receives unit-normalised, account-relative evidence and can interpret, support, oppose, or abstain, but its opinions are advisory: it cannot invent a contract, set position size, or grant or deny execution permission. Final authority belongs to a deterministic risk constitution covering account status, options approval, defined-risk geometry, expiry, liquidity, quote freshness, drawdown, concentration, duplicate exposure, market window, and Monte Carlo stress. Simulation can veto a proposal; it can never promote one.
