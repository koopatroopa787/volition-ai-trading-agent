...
function SummaryStrip({ data }: { data: Dashboard }) {
  ...
  const items = [
    ...
    ["Capital at risk", percentage(openRisk, 2), `${money(data.account.open_risk)} across ${data.account.open_structures} structures`],
  ];
  return <section className="summary-strip" aria-label="Account summary">{items.map(([label, value, detail], index) => <div key={label}><span>{label}</span><strong className={index === 1 ? (data.account.daily_pnl >= 0 ? "gain" : "loss") : label === "Capital at risk" && data.account.open_risk_breached ? "loss" : ""}>{value}</strong><small>{label === "Capital at risk" && data.account.open_risk_breached ? `${detail} · ${percentage(data.account.open_risk_limit_pct)} cap exceeded` : detail}</small></div>)}</section>;
}
